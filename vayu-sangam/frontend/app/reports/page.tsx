...
function aqiBgColor(aqi: number) {
...
export default function ReportsPage() {
  ...
  const cityStats = useMemo(() => {
    ...
    const cityMap: Record<string, { count: number; aqiSum: number; pm25Sum: number; pm10Sum: number; no2Sum: number; maxAqi: number; maxAqiStation: string }> = {};

    data.forEach(row => {
      const city = row.city || 'Unknown';
      if (!cityMap[city]) {
        cityMap[city] = { count: 0, aqiSum: 0, pm25Sum: 0, pm10Sum: 0, no2Sum: 0, maxAqi: 0, maxAqiStation: '' };
      }
      cityMap[city].count++;
      cityMap[city].aqiSum += row.aqi || 0;
      cityMap[city].pm25Sum += row.pm25 || 0;
      cityMap[city].pm10Sum += row.pm10 || 0;
      cityMap[city].no2Sum += row.no2 || 0;
      
      if ((row.aqi || 0) > cityMap[city].maxAqi) {
        cityMap[city].maxAqi = row.aqi;
        cityMap[city].maxAqiStation = row.station_name;
      }
    });

    return Object.keys(cityMap).map(city => ({
      city,
      stationCount: cityMap[city].count,
      avgAqi: Math.round(cityMap[city].aqiSum / cityMap[city].count),
      avgPm25: Math.round(cityMap[city].pm25Sum / cityMap[city].count),
      avgPm10: Math.round(cityMap[city].pm10Sum / cityMap[city].count),
      avgNo2: Math.round(cityMap[city].no2Sum / cityMap[city].count),
      maxAqi: cityMap[city].maxAqi,
      maxAqiStation: cityMap[city].maxAqiStation
