...
export default function KpiCards({ forecast }: { forecast: any[] }) {
  const metrics = useMemo(() => {
    ...
    const pm25Sparkline: { value: number }[] = [];
    const o3Sparkline: { value: number }[] = [];

    forecast.forEach(f => {
      pm25Sum += f.pm25_ug_m3 ?? 0;
      if (f.o3_ppb > maxO3) maxO3 = f.o3_ppb;
      
      pm25Sparkline.push({ value: f.pm25_ug_m3 ?? 0 });
      o3Sparkline.push({ value: f.o3_ppb ?? 0 });
    });
    ...
    return {
      ...
      currentO3: Math.round(forecast[0].o3_ppb ?? 0),
